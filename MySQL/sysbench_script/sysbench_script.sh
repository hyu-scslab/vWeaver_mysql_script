...
# prepare sysbench
SYSBENCH_OPT=""
SYSBENCH_OPT+=" --db-driver=mysql --mysql-host=localhost --mysql-user=root --report-interval=1 --secondary=off --create_secondary=off --mysql-port=${PORT}"
# SYSBENCH_OPT+=" --mysql-password=sbtest"
SYSBENCH_OPT+=" --mysql-socket=$MYSQL_BASE/mysql.sock"
SYSBENCH_OPT+=" --time=200"
SYSBENCH_OPT+=" --threads=1"
SYSBENCH_OPT+=" --tables=5" 
...
# $SYSBENCH $SYSBENCH_OPT $SYSBENCH_WORKLOAD "run"
